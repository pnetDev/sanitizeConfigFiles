...
Log changes here:
11/05/18 CM Working on code so we can keep the newest file
...
for backupFile in fileNameList[1:]:                                       ## We start the for at the second item in the list. Python starts counting at zero
	...
	backupFileName = dir_path + backupFile
        ...
        if compare == True:
                ...
                print ""
		lastFileName = backupFileName                                           ## So we can track the newest file before the change.
        if compare == False:
		...
		print "\nDEBUG: Newest file before change: " ,lastFileName
		print "\n"
		newestFlagPreserve.append(lastFileName)
